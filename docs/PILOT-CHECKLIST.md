...
### Cách test nhanh trial

1. Tài khoản mới + confirm (nếu bật) + login.
2. Nhập ngày sinh → kiểm tra row `profiles` + banner trial trên Hôm nay / tab Pro.
3. Tạo tử vi nhiều lần trong trial — không bị limit.
4. (Optional) SQL: `update profiles set trial_ends_at = now() - interval '1 minute' where id = '…'` → reload → modal hết trial + Free limits + ads.
5. Build với `STORE_BUILD=1` hoặc release: không thấy switch “Mở khóa Pro (demo)”.
